...
! 2007: ECH added dynamic file units, modified from POP_IOUnitsMod.F90
      ...
      integer (kind=int_kind), public :: &
         ...
         nu_diag_l          ! diagnostics output file, local to each proc

      character (32), public :: &
         nml_filename = 'ice_in' ! namelist input file name

      character (32), public :: &
         ldiag_filename          ! local diag file name
...
!  nu_nml is obtained separately.
      ...
      use ice_communicate, only:my_task
         ...
         call get_fileunit(nu_diag_l)

         write(ldiag_filename,'(a11,i0)') 'cice.ldiag.',my_task

         open(nu_diag_l,file=ldiag_filename,status='new',iostat=lerr)
...
!  This routine releases unit numbers at the end of a run. 
      ...
      subroutine release_all_fileunits
         ...
         close(nu_diag_l,status='KEEP')
         call release_fileunit(nu_diag_l)
