...
!       horizontal_remap
      ...
      save
      private
      public :: init_remap, horizontal_remap, make_masks, dpx, dpy, dsx, dsy
      ...
      real (kind=dbl_kind), dimension(:,:,:), allocatable :: dpx, dpy, dsx, dsy
...
!                          xxxav, xxyav, xyyav, yyyav
      ...
      integer (kind=int_kind) :: alloc_err
...
!            yyyav(i,j,iblk) = c0
      ...
      allocate(dpx(nx_block,ny_block,max_blocks),STAT=alloc_err)
      allocate(dpy(nx_block,ny_block,max_blocks),STAT=alloc_err)
      allocate(dsx(nx_block,ny_block,max_blocks),STAT=alloc_err)
      allocate(dsy(nx_block,ny_block,max_blocks),STAT=alloc_err)
...
!                          xyav, xxxav, xxyav, xyyav, yyyav
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block,max_blocks), intent(out) ::     &
         ...
         dsx            ,&! x coordinates of downstream points at cell corners
         dsy              ! y coordinates of downstream points at cell corners
...
!                                  xyyav    (:,:,iblk), yyyav  (:,:,iblk),   &
    ...
    !-------------------------------------------------------------------
         ...
         call downstream_points(nx_block,         ny_block,          &
                                ilo, ihi,         jlo, jhi,          &
                                nghost,           dt,                &
                                uvel  (:,:,iblk), vvel(:,:,iblk),    &
                                dxu   (:,:,iblk), dyu (:,:,iblk),    &
    ...
    !-------------------------------------------------------------------
      ...
      if (nghost==1) then
         ...
         call ice_HaloUpdate (dpy,                halo_info, &
                              field_loc_NEcorner, field_type_vector)
         ! downstream points
         call ice_HaloUpdate (dsx,                halo_info, &
                              field_loc_NEcorner, field_type_vector)
         call ice_HaloUpdate (dsy,                halo_info, &
                              field_loc_NEcorner, field_type_vector)
...
! author William H. Lipscomb, LANL
      ...
      subroutine downstream_points (nx_block,   ny_block,   &
                                    ilo, ihi,   jlo, jhi,   &
                                    nghost,     dt,   &
                                    uvel,       vvel,    &
                                    dxu,        dyu,     &
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block), intent(out) ::   &
         dsx           ,&! coordinates of downstream points (m)
         dsy             ! coordinates of downstream points (m)
      ...
      real (kind=dbl_kind) ::                  &
         mpx,  mpy      ,&! coordinates of midpoint of forward trajectory,
                          ! relative to cell corner
         mpxt, mpyt     ,&! midpoint coordinates relative to cell center
         ump,  vmp        ! corrected velocity at midpoint
    ...
    !-------------------------------------------------------------------
      ...
      do i = ilo-nghost+1, ihi+nghost-1
         ...
         ! Check for values out of bounds (more than one grid cell away)
         if (dsx(i,j) < -HTN(i,j) .or. dsx(i,j) > HTN(i+1,j) .or.   &
             dsy(i,j) < -HTE(i,j) .or. dsy(i,j) > HTE(i,j+1)) then
            l_stop = .true.
            istop = i
            jstop = j
         endif
      ...
      if (l_stop) then
         ...
         write (nu_diag,*) ' '
         write (nu_diag,*)   &
                    'Warning: Downstream points out of bounds in remap'
         write (nu_diag,*) 'my_task, i, j =', my_task, i, j
         write (nu_diag,*) 'dsx, dsy =', dsx(i,j), dsy(i,j)
         write (nu_diag,*) 'HTN(i,j), HTN(i+1,j) =', HTN(i,j), HTN(i+1,j)
         write (nu_diag,*) 'HTE(i,j), HTE(i,j+1) =', HTE(i,j), HTE(i,j+1)
         return
      endif

      if (l_dp_midpt) then ! find downstream pts using corrected midpt velocity 
    ...
    !-------------------------------------------------------------------
    ! Scale downstream points to coordinate system in which grid cells
    ! have sides of unit length.
    !-------------------------------------------------------------------

            dsx(i,j) = dsx(i,j) / dxu(i,j)
            dsy(i,j) = dsy(i,j) / dyu(i,j)

    !-------------------------------------------------------------------
    ! Estimate midpoint of forward trajectory relative to corner (i,j).
    !-------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------
      ...
      end subroutine downstream_points
